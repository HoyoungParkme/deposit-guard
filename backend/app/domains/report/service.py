...
class ReportService:
    ...
    def pick_todos(self, inp: ReportInput) -> list[Todo]:
        ...
        unknowns = {u.code for u in inp.check.unknowns}
        mortgages = [
            e
        ...
        signal_labels = {s.code: s.label for s in inp.check.signals}
        unknown_labels = {u.code: u.how_to_check for u in inp.check.unknowns}
    ...
    async def write(
        ...
        for sig in inp.check.signals:
            if sig.code not in explanations:
                explanations[sig.code] = base.explanations.get(sig.code, sig.label)

        questions = list(s.questions)
        if len(questions) < 3:
            for bq in base.questions:
                if bq not in questions:
                    questions.append(bq)
                if len(questions) >= 3:
                    break
        questions = questions[:5]
        ...
        if factcheck.contradicts(conclusion, allowed_manwon, allowed_ratios, grade_str):
        ...
        for code, exp in list(explanations.items()):
            if factcheck.contradicts(exp, allowed_manwon, allowed_ratios, grade_str):
                explanations[code] = base.explanations.get(code, exp)
                corrections += 1
        ...
        # 1) 인용 지우기
        await self._citation_service.clear_report(review_id)

        # 2) 결론 표식 치환
        cited = await self._citation_service.resolve_markers(
            review_id, conclusion, CitationUse.conclusion, ""
        )
        ...
        await self._citation_service.cite(
            review_id, CitationUse.rights, "", rights_label, inp.rights.based_on
        )
        ...
        for sig in inp.check.signals:
            if sig.entry_ids:
                await self._citation_service.cite(
                    review_id, CitationUse.signal, sig.code, sig.label, sig.entry_ids
                )

        for chk in inp.check.checked:
            if chk.entry_ids:
                await self._citation_service.cite(
                    review_id, CitationUse.checked, chk.code, chk.label, chk.entry_ids
                )

        for idx, (cl, c_entries) in enumerate(zip(clauses, clause_entries)):
            if c_entries:
                await self._citation_service.cite(
                    review_id, CitationUse.clause, str(idx + 1), cl.title, c_entries
                )

        # 5) 판 번호 계산
        prev_rev = await crud.get_revision_no(self._session, review_id)
    ...
    async def get(self, review_id: str) -> Report:
        """의견서와 인용을 조회한다.

        항목 ID: JSD-MS-008#ReportService.get
        근거: JSD-SEQ-001#SEQ-14, JSD-API-001#GET/api/reviews/{id}/report
        """
        op = await crud.get_opinion(self._session, review_id)
        if not op:
            raise AppError("not_found", "의견서가 아직 작성되지 않았습니다.")

        refs = await self._citation_service.for_report(review_id)
        body = dict(op.body)
        ...
        rights = RightsSummary(
            senior_mortgage_manwon=r_data["senior_mortgage_manwon"],
            senior_lease_manwon=r_data["senior_lease_manwon"],
            other_tenants_manwon=r_data["other_tenants_manwon"],
            senior_total_manwon=r_data["senior_total_manwon"],
            deposit_manwon=r_data["deposit_manwon"],
    ...
    async def exists(self, review_id: str) -> bool:
        ...
        """
        return await crud.exists_opinion(self._session, review_id)

    async def shareable(self, review_id: str) -> Shareable:
        """공유본에 담을 사본을 만든다.
        ...
        # 의견서 로드
        report = await self.get(review_id)
    ...
    async def delete_for_review(self, review_id: str) -> None:
        """의견서를 삭제한다.

        항목 ID: JSD-MS-008#ReportService.delete_for_review
        """
        await crud.delete_opinion(self._session, review_id)
