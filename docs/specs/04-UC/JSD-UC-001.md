...
| LLM | 보조 (외부) | 에이전트 판단, 추출 구조화, 의견서 문장 생성 |
...
| 수준 | 사용자 목표 |
| 주 액터 | 세입자 |
| 이해관계자와 관심사 | 세입자: 계약 전 위험을 알고 준비 목록을 받는다 / 운영자: 비용 한도 안에서 처리된다 / 임대인: 개인정보가 저장되지 않는다 |
...
3. 시스템이 검토 세션을 만들고 검토 중 화면으로 전환한다
...
- **2c. 같은 파일 해시가 캐시에 있음**
  - 2c1. 캐시된 검토 기록을 같은 순서로 재생하고 의견서를 보여준다. "앞서 처리된 결과" 표시. 9로
- **4a. 등기부 읽기 실패 (파싱 불가·등기부가 아님)**
  - 4a1. "등기부로 읽히지 않습니다. 인터넷등기소 PDF 원본을 올려주세요" 안내. 세션 종료, 9로
- **4b. 도구 호출이 20회에 달함**
...
| 이해관계자와 관심사 | 심사위원: 3분 안에 실제 동작과 근거를 본다 / 운영자: 예시는 비용이 거의 들지 않는다 |
| 사전조건 | 없음 |
...
4. 첫 처리라면 실제로 도구가 돈다. 이미 처리된 파일이면 [[#UC-A1]] 2c로 캐시 재생

**확장**
- **2a. 사용자가 보증금을 바꿈**
  - 2a1. 파일 해시 + 입력값이 캐시 키이므로 새 조합은 실제 처리된다
- **4a. 배포 직후 캐시가 비어 있음**
  - 4a1. 운영자가 배포 후 예시 3건을 한 번씩 돌려 캐시를 채운다 (운영 절차, INFRA)
...
- **2b. 토지 등기부 업로드 요청인 경우**
  - 2b1. 카드에 파일 입력이 있고, 올리면 [[#UC-S1]]이 두 번째 문서로 다시 돈다
- **2c. 5분 동안 답이 없음**
...
**기본 흐름**
1. 세입자가 "PDF 저장"을 누른다
2. 시스템이 의견서를 PDF로 만들어 내려준다. 등기부 원문 하이라이트는 포함하지 않고 근거 문장만 넣는다
3. 세입자가 "링크 공유"를 누른다
...
| 사전조건 | 파일이 검사를 통과했다 |
| 최소 보장 | 실패 시 사유 문자열을 돌려준다. 부분 추출이면 `null` 필드와 함께 돌려준다 |
| 성공 보장 | 표제부·갑구·을구 항목 JSON + 각 항목의 원문 위치 + 문서 종류(건물/토지/집합) |
| 트리거 | 에이전트 첫 호출 또는 토지 등기부 추가 시 |
| 패키지 | 도구 |
| 포함(include) | — |
| 확장점(extension point) | — |
| 일반화 | — |
...
**기본 흐름**
1. 문서 파싱 서비스에 파일을 보내 표 구조가 보존된 텍스트(HTML 또는 마크다운)와 블록 위치를 받는다
2. 표제부·갑구·을구 구간을 헤더로 나눈다
3. 구간별 표를 행 단위로 읽어 LLM에 구조화를 맡긴다. 출력 스키마: 순위번호, 등기목적, 접수일·번호, 등기원인·거래가액, 권리자, 금액(채권최고액·전세금), 말소 여부, 부기 대상 순위, 원문 블록 ID
4. 말소 표시(취소선·"말소" 등기목적)를 `말소=true`로, 부기등기(1-1)를 주등기에 연결한다
5. 표제부에서 건물 종류·집합건물 여부·대지권 미등기·토지 별도등기 표시를 뽑는다
6. 필수 필드 누락을 세어 `warnings`에 넣고 결과를 돌려준다

**확장**
- **1a. 파싱 서비스 실패·타임아웃(30초)**
  - 1a1. 1회 재시도. 다시 실패하면 실패 사유 반환 → [[#UC-A1]] 4a
- **2a. 갑구·을구 헤더가 없음 (등기부가 아님)**
  - 2a1. "등기부 아님" 반환
- **3a. LLM 출력이 스키마와 맞지 않음**
  - 3a1. 1회 재요청. 다시 실패하면 해당 구간 `null` + warning
- **5a. 건물 종류를 판정하지 못함**
...
**기본 흐름**
1. 시스템 프롬프트: 역할(LH 권리분석 담당자), 도구 8종 설명, 검토 항목 목록, 건물 종류별 필수 검토([[JSD-PRD-001#R11]]), 금지 사항(도구 밖 사실, 등급 변경), 진행 문장 형식
2. 첫 호출은 반드시 [[#UC-S1]]
...
| 성공 보장 | 통과한 요청만 검토 세션이 되고, 캐시 적중이면 처리 없이 응답한다 |
| 트리거 | [[#UC-A1]] 2 |
| 패키지 | 운영 |
| 포함(include) | — |
| 확장점(extension point) | — |
...
**기본 흐름**
1. 파일 형식·크기·페이지 수 검사
2. 파일 해시 + 보증금 + 계약 형태 + 상대방 이름으로 캐시 키를 만든다
3. 캐시 적중이면 저장된 검토 기록과 의견서를 돌려준다
