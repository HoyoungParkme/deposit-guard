...
출처: [[JSD-PRD-001#N1]]. 원본 파일은 디스크에 남기지 않는다. 파싱 텍스트는 근거 하이라이트에 필요하므로 검토를 만든 뒤 24시간까지만 DB에 두고 지운다(같은 파일의 파싱 캐시도).
...
출처: 의뢰인 결정 2026-09-15. OpenAI API 직접 호출. 에이전트 판단·의견서 문장 모두 같은 모델. 등기부 구조화에는 모델을 쓰지 않는다. 모델 ID는 환경변수로 두어 교체 가능하게 한다.
...
요청 흐름: 업로드 → 요청 안에서 업스테이지 파싱(같은 파일이면 파싱 캐시, DB 트랜잭션 밖) → 세션 생성·파싱 결과 저장(DB), 파일 바이트 폐기 → 백그라운드 태스크로 에이전트 루프 → 이벤트를 DB에 적재하며 SSE로 밀어냄 → 질문이 나오면 세션 상태 `waiting_user`로 멈춤 → 답변 API가 오면 재개 → 의견서 저장. 검토에 딸린 데이터는 만든 뒤 24시간에 삭제.
...
| 개인정보 | 로그에 이름·주민번호·상세주소 금지 (로그 필터), 파싱 텍스트는 검토를 만든 뒤 24시간에 삭제 (같은 파일의 파싱 캐시도) |
...
DB 볼륨은 compose named volume. 백업은 하루 1회 `pg_dump`를 Azure Blob에 올리는 스크립트 (운영 절차, 미결 9 참고). 보관 기한이 있는 테이블(검토에 딸린 것 전부, `file_cache`, `shares`, `lookup_cache`)은 `--exclude-table-data`로 데이터를 빼고 스키마만 담는다 — 백업이 24시간·7일 삭제보다 오래 붙들지 않게. 기한이 없는 `usage_log`·`hug_defaulters`는 데이터째 담는다.
...
| 합계 | | **약 26~31만 원** (캐시 적중과 예시 파일은 파싱 비용만 줄인다 — 에이전트는 매번 돈다) |
...
- [x] Document Parse 응답의 블록 ID·좌표 형식 — 9/15 첫 호출로 확인, 하이라이트 방식 확정 — 결정: 2026-09-16 출력 확인. 요소마다 id가 붙어 블록 ID로 강조한다. 좌표는 쓰지 않는다 (3장 원문 하이라이트)
- [x] 파싱 결과 보관 기간 24시간이 적절한지 (의견서 재열람 시 근거 하이라이트가 사라지는 시점) — 결정: 24시간. 의견서를 포함해 검토에 딸린 것을 모두 만든 뒤 24시간에 함께 지우므로 의견서만 남아 하이라이트를 잃는 경우가 없다
