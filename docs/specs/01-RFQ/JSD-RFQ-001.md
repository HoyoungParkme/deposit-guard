...
RFQ → PRD → SCN → UC → INFRA → DOM → UI → API → SEQ → MS → CODE. 6일 일정이므로 단계마다 1문서, 항목 수는 최소로 유지한다. 이 문서는 "무엇을"만 적는다. 판정 규칙의 확정값·파이프라인 구조·기술 스택은 PRD와 INFRA에서 정한다.
...
#### Q19 계산은 코드가, 해석은 AI가 한다

의뢰인 합의: 판단이 틀리면 보증금 전액을 잃는 영역이다. 금액 합산·비율·등급 판정은 결정적 규칙으로 처리하고, AI는 문서 읽기(추출)와 설명문 생성만 맡는다. AI가 낸 추출값은 사용자가 화면에서 고칠 수 있어야 한다.
...
flowchart LR
    ...
    B --> C[권리관계 추출<br/>+ 원문 위치]
    C --> D[위험도 계산<br/>+ 신호 판정<br/>+ 임대인 조회]
    D --> E[등급·근거·설명]
    E --> F[체크리스트<br/>특약·질문<br/>저장·공유]
```
...
| 09-15 | 등기부 파싱·추출 파이프라인 (샘플 3건 정확 추출) |
| 09-16 | 위험 계산·신호 판정·HUG·실거래가 연동 |
| 09-17 | 화면 3개, 근거 하이라이트, 샘플 체험 |
