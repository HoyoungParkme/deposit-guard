...
#### Q13 로그인 없이 예시 등기부 파일로 즉시 체험한다

심사위원과 투표자는 자기 등기부가 없다. 첫 화면에서 가상 등기부 PDF 파일(안전·주의·위험 각 1건)을 골라 올리면 **실제 에이전트가 실제로 처리**해 의견서까지 간다. 의뢰인 결정(2026-09-15): 결과를 미리 만들어 재생하는 샘플은 두지 않는다. 회원가입·로그인은 두지 않는다.
...
공개 기간 4주 동안 유료 API 비용은 의뢰인 부담이다. 파일 크기·페이지 수·IP당 일일 횟수를 제한하고, 같은 문서(파일 해시 기준)는 다시 처리하지 않고 캐시 결과를 준다. 예시 등기부 파일(Q13)도 실제 처리하지만 해시가 같으므로 두 번째부터는 캐시로 응답해 비용을 막는다.
...
```mermaid
flowchart TD
    A[등기부 PDF 업로드<br/>예시 파일 제공] --> B[보증금·계약형태 입력]
...
이번 범위에 넣지 않는 것: 회원가입, 결제, 중개사용 관리 화면, 계약서 전체 검토, 입주 후 분쟁 대응, 보증보험 신청 대행, 다가구 선순위 보증금 자동 조회(전입세대 열람은 본인만 가능), 미리 계산된 결과를 재생하는 샘플. 잔금일 등기부 비교(Q39)는 선택.

| 날짜 | 목표 |
|---|---|
| 09-14 | 접수, API 키 신청, 샘플 등기부 확보, 명세 RFQ·PRD |
| 09-15 | 등기부 읽기 도구 (예시 3건 정확 추출) + 계산·규칙 도구 |
| 09-16 | 에이전트 루프 + 시세·건축물대장·명단 도구 + 사용자 질문 |
| 09-17 | 화면 3개, 진행 상황 표시, 근거 하이라이트, 예시 파일 |
...
- [ ] 예시 등기부 PDF 3건 제작 방법 (실제 문서 마스킹 vs 가상 제작). 갑구·을구 본문 샘플 텍스트는 공개 자료에서 찾지 못함
