...
**주체**: P1
**상황**: 서울 강서구 다세대(집합건물) 3층 301호, 2025년 보존등기. 을구 근저당 1건 채권최고액 2억 1천(2025년 설정, 검토일 기준 60일 전). 갑구에 2025년 매매 거래가액 3억 6천. 보증금 1억 8천, 전세. 신축이라 같은 건물 실거래 없음.

1. 업로드, 보증금 1억 8천, 전세
2. 진행: 표제부에서 집합건물·다세대 판정, 대지권 등기 정상 → "갑구: 2025년 보존등기, 같은 해 매매 1건" → "을구: 근저당 2억 1천, 60일 전 설정"
3. "실거래가를 찾습니다" → 없음 → "등기부의 올해 매매가 3억 6천을 씁니다"
4. "건축물대장을 조회합니다" → 주용도 다세대주택, 세대수 12
5. 질문 카드: "정부24 건축물대장에 '위반건축물' 표시가 있나요?" (확인 방법 링크) → 사용자 "아니오"
6. 합산: (2.1억 + 1.8억) ÷ 3.6억 = **108%**… 이 값이면 위험이므로, 예시 파일은 매매가 5억으로 둔다 → (2.1 + 1.8) ÷ 5.0 = 78%, 선순위비율 42%
...
4. "토지 등기부 을구: 근저당 1억"
5. "건축물대장: 다가구주택, 6가구" → 질문: "이 건물에 다른 세입자가 몇 가구 있고, 아는 보증금 합계가 있나요?" → "4가구, 모름"
...
**변형 — 두 번째 사람이 같은 예시를 돌림**: 파일 해시 캐시로 즉시 응답하되, 진행 화면은 기록된 이벤트를 같은 순서로 보여준다 (재계산 없음, 결과는 실제 처리 결과의 캐시). 화면에 "이 예시는 앞서 처리된 결과입니다" 표시.
...
| S4 심사위원 | (S2·S3 재사용) | — | 캐시 | G1 R2 R12 R13 N2 N3 N4 |
