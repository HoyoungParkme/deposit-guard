...
flowchart LR
    ...
    A -->|호출| T[도구 8종<br/>읽기·합산·규칙·조회·질문·작성]
    T -->|결과만| A
    A -->|진행 상황| S[화면]
...
근거: [[JSD-RFQ-001#Q4]] [[JSD-RFQ-001#Q13]]. 로그인 없이 샘플 버튼 하나로 검토 과정과 의견서까지 도달한다.
...
첫 화면에 샘플 3건(안전·주의·위험) 버튼. 누르면 업로드 없이 실제와 같은 검토 과정이 재생되고 의견서에 도달한다. 결과는 미리 계산해 두고, 진행 표시는 실제 속도에 가깝게 재생한다.

- [ ] 샘플 3건 모두 60초 안에 의견서 도달
- [ ] 샘플에는 실제 개인정보가 없다 (가명·가상 주소)
- [ ] 샘플도 근거 하이라이트가 동작한다
...
PDF·이미지를 표 구조를 보존해 파싱하고, 표제부·갑구·을구를 항목 단위 JSON으로 만든다. 각 항목에 원문 위치(페이지·블록 또는 좌표)를 붙인다. 말소된 항목은 `말소=true`로 남기고 버리지 않는다. 부기등기(1-1 등)는 주등기에 연결한다.
...
- [ ] 샘플 등기부 3건에서 위 필수 필드 전부를 사람이 검수한 정답과 일치하게 뽑는다
- [ ] 말소 항목이 합산에서 빠진다
...
- [ ] 샘플 3건(아파트·다세대·다가구)에서 표의 필수 항목이 모두 시도된 기록이 남는다
...
| 화면 | 내용 |
|---|---|
| 시작 | 한 줄 설명, 샘플 3개 버튼, 업로드·입력 폼, 미저장 고지 |
| 검토 중 | 진행 상황 스트림, 질문 카드 (R8), 취소 |
| 의견서 | 등급 배지, 결론, 신호(근거 보기 → 원문 하이라이트 패널), 단계별 할 일(탭), 특약·질문(복사), 검토 기록, 판정 기준 링크, 저장·공유 |
...
- [ ] IP당 하루 5건, 파일 10MB·20페이지
- [ ] 같은 파일 해시는 재처리하지 않고 캐시 결과를 준다
...
| 지표 | 목표 | 측정 |
|---|---|---|
| 샘플 체험 도달 시간 | 버튼 → 의견서 60초 이내 | 샘플 3건 실측 |
| 추출 정확도 | 샘플 3건 필수 필드 100% | 정답표 대조 |
...
- [ ] 샘플 등기부 3건 확보 (실제 문서 마스킹 vs 가상 제작)
